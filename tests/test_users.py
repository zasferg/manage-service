...
from core.auth import hash_password, create_access_token, create_refresh_token
from infrastructure.schemas.schemas import UserUpdate, User
...
async def test_get_user(test_session, test_user):
    ...
    current_user = await UsersService(test_session).get_current_user(current_user=authenticated_user)
...
async def test_delete_user(test_session, test_user):
    ...
    await UsersService(test_session).delete_current_user(current_user=authenticated_user)
